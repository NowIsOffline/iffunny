    ...
    return (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
            <div className="bg-white rounded-xl shadow-lg w-[32rem] max-h-[80vh] overflow-y-auto p-4">
                ...
                <ul className="space-y-3 overflow-y-auto max-h-[60vh]"> {/* Added fixed height and scrolling */}
                        ...
                        .map(app => (
                            <li key={app.id} className="flex items-center justify-between border p-2 rounded-md hover:bg-gray-50">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div className="text-left"> {/* Ensure text aligns to the left */}
                                ...
                                <button
                                    ...
                                    className={`px-4 py-1 rounded-md text-white ${installedIds.includes(app.id) ? 'bg-green-500' : 'bg-red-500'} flex items-center justify-center`}
                                    style={{width: '100px'}} // Adjust width to match button's size
                                >
                                    {installedIds.includes(app.id) ? 'Install' : 'Uninstall'}
                                </button>
                            </li>
                        ))}
                </ul>
